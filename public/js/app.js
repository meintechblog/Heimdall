...
(function registerDashboardFilters(root, factory) {
  ...
  function updateTagButtonCounts() {
    ...
    var datasetItems = items.filter(function filterBySearch() {
  ...
  function applyTileFilters() {
    ...
    items.filter(function filterItems() {
      ...
      var matchesTag = selectedTag === ALL_TAG || (String(selectedTag).startsWith("cat-") ? item.closest(".category").hasClass(selectedTag) : item.hasClass(selectedTag));
  ...
  $("#search-container").on("input.dashboardFilters", "input[name=q]", function () {
    applyTileFilters();
  }).on("change.dashboardFilters", "select[name=provider]", function toggleSubmit() {
    if ($(this).val() === "tiles") {
