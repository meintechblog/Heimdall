...
class ShellyDiscoveryService
{
    ...
    protected function cachedCandidates(): array
    {
        ...
        $lock = Cache::lock(self::LOCK_KEY, max(10, (int) config('app.discovery.shelly.cache_ttl_seconds', 900)));
        ...
        try {
            ...
            Cache::put(self::CACHE_KEY, $candidates, now()->addSeconds((int) config('app.discovery.shelly.cache_ttl_seconds', 900)));
    ...
    protected function scanCandidates(): array
    {
        ...
        foreach (array_chunk($hosts, max(1, (int) config('app.discovery.shelly.chunk_size', 4))) as $chunk) {
            $responses = Http::pool(function (Pool $pool) use ($chunk) {
                ...
                foreach ($chunk as $host) {
                    $requests[] = $pool
                        ->as($host.':device')
                        ->timeout((float) config('app.discovery.shelly.timeout_seconds', 0.8))
                        ->connectTimeout((float) config('app.discovery.shelly.connect_timeout_seconds', 0.4))
                        ->acceptJson()
                        ->get("http://{$host}/shelly");
                    $requests[] = $pool
                        ->as($host.':settings')
                        ->timeout((float) config('app.discovery.shelly.timeout_seconds', 0.8))
                        ->connectTimeout((float) config('app.discovery.shelly.connect_timeout_seconds', 0.4))
            ...
            foreach ($chunk as $host) {
                $deviceResponse = $responses[$host.':device'] ?? null;
                $settingsResponse = $responses[$host.':settings'] ?? null;

                if (! $deviceResponse instanceof Response || ! $deviceResponse->successful()) {
                    continue;
                }

                $devicePayload = $deviceResponse->json();

                if (! is_array($devicePayload) || ! $this->isShellyPayload($devicePayload)) {
                    continue;
                }
                ...
                $title = $this->candidateTitle($host, $devicePayload, $settingsPayload);
                $subtitle = $this->candidateSubtitle($devicePayload, $settingsPayload);
                $url = "http://{$host}";
    ...
    {
        $name = trim((string) ($settingsPayload['name'] ?? ''));

        if ($name !== '') {
            return $name;
        }

        $hostname = trim((string) data_get($settingsPayload, 'device.hostname', ''));
    ...
    {
        $configuredHosts = array_filter((array) config('app.discovery.shelly.hosts', []));
    ...
    protected function forgetCandidate(string $candidateId): void
    {
        ...
        Cache::put(self::CACHE_KEY, $remainingCandidates, now()->addSeconds((int) config('app.discovery.shelly.cache_ttl_seconds', 900)));
    }
    ...
    protected function targetTagId(): int
    {
        $tag = Item::query()
            ...
            ->where(function ($query) {
                $query->where('url', 'shelly')
                    ->orWhere('title', 'Shelly');
            })
            ->orderByDesc('pinned')
            ->first();
