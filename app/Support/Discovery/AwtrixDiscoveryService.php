...
class AwtrixDiscoveryService
{
    protected const CACHE_KEY = 'discovery:awtrix:candidates';
    protected const LOCK_KEY = 'discovery:awtrix:scan';
    ...
    protected function cachedCandidates(): array
    {
        ...
        $lock = Cache::lock(self::LOCK_KEY, max(10, (int) config('app.discovery.awtrix.cache_ttl_seconds', 900)));
        ...
        try {
            ...
            Cache::put(self::CACHE_KEY, $candidates, now()->addSeconds((int) config('app.discovery.awtrix.cache_ttl_seconds', 900)));
    ...
    protected function scanCandidates(): array
    {
        ...
        foreach (array_chunk($hosts, max(1, (int) config('app.discovery.awtrix.chunk_size', 4))) as $chunk) {
            $responses = Http::pool(function (Pool $pool) use ($chunk) {
                ...
                foreach ($chunk as $host) {
                    $requests[] = $pool
                        ->as($host)
                        ->timeout((float) config('app.discovery.awtrix.timeout_seconds', 0.8))
                        ->connectTimeout((float) config('app.discovery.awtrix.connect_timeout_seconds', 0.4))
                        ->acceptJson()
            ...
            foreach ($chunk as $host) {
                ...
                if (! is_array($payload) || ! $this->isAwtrixPayload($payload)) {
                    continue;
                }
                ...
                if ($version !== '') {
                    $subtitleParts[] = "AWTRIX {$version}";
                }

                if ($currentApp !== '') {
                    $subtitleParts[] = $currentApp;
                }

                $candidates[] = [
                    'id' => sha1('awtrix:http://'.$host),
                    'source' => 'awtrix',
                    'sourceLabel' => $this->label(),
                    'title' => "AWTRIX {$host}",
                    'subtitle' => $subtitleParts !== [] ? implode(' · ', $subtitleParts) : 'AWTRIX',
                    'url' => "http://{$host}",
                    'host' => $host,
                    'icon' => null,
    ...
    protected function isAwtrixPayload(array $payload): bool
    {
        $uid = strtolower(trim((string) ($payload['uid'] ?? '')));

        return str_starts_with($uid, 'awtrix_');
    }

    protected function candidateHosts(): array
    {
        $configuredHosts = array_filter((array) config('app.discovery.awtrix.hosts', []));
    ...
    protected function forgetCandidate(string $candidateId): void
    {
        ...
        Cache::put(self::CACHE_KEY, $remainingCandidates, now()->addSeconds((int) config('app.discovery.awtrix.cache_ttl_seconds', 900)));
    }
    ...
    protected function targetTagId(): int
    {
        $tag = Item::query()
            ...
            ->where(function ($query) {
                $query->where('url', 'awtrix')
                    ->orWhere('title', 'AWTRIX')
                    ->orWhere('title', 'Ulanzi');
            })
            ->orderByDesc('pinned')
            ->first();
